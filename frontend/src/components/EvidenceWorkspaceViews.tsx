          ...
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-2"><Label>Selected documents <span className="text-muted-foreground">({selectedDocuments.length})</span></Label>
              <div className="max-h-52 space-y-1 overflow-y-auto rounded-md border p-2">
                {documentsQuery.isLoading ? <div className="p-2 text-sm text-muted-foreground">Loading documents…</div> : documents.length === 0 ? <div className="p-2 text-sm text-muted-foreground">Upload documents before adding them to a workspace.</div> : documents.map((document) => <button key={document.id} type="button" onClick={() => toggleDocument(document.id)} className={`flex w-full items-center justify-between gap-3 rounded px-2 py-2 text-left text-sm ${selectedDocuments.includes(document.id) ? 'bg-primary/10 text-primary' : 'hover:bg-accent'}`}><span className="truncate">{document.filename}</span>{documentStatus(document)}</button>)}
              </div>
            </div>
          </div>
...
function WorkspaceChat({ workspace }: { workspace: EvidenceWorkspace }) {
  ...
  const queryMutation = useMutation({
    ...
    onSuccess: (result, query) => setTurns((current) => [...current, { id: result.query_id, query, result }]),
    onError: (error) => toast.error('Workspace query failed', { description: getApiErrorMessage(error) }),
  })
  const ask = () => {
...
export function EvidenceWorkspaceDetailView() {
  ...
  const availableDocuments = useMemo(() => {
    ...
    return (documentsQuery.data?.documents ?? []).filter((document) => !included.has(document.id))
